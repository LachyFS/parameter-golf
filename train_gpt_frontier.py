"""
Frontier training script combining all top techniques:
- Int6 quantization + zstd-22 compression
- MLP 3x expansion (hidden=1536)
- FP16 passthrough for tied embedding + late-layer key projections
- SmearGate + BigramHash embedding
- STE fake-int6 QAT (quantization-aware training)
- NorMuon optimizer (per-row RMS normalization after Newton-Schulz)
- Sliding window evaluation (stride=64)
- Orthogonal initialization + muP-scaled output projections
- Stochastic Weight Averaging (SWA) during warmdown
- Tuned hyperparameters from 60+ competition experiments
"""
...
def zeropower_via_newtonschulz5(G: Tensor, steps: int = 10, eps: float = 1e-7) -> Tensor:
    # NorMuon: Newton-Schulz orthogonalization + per-row RMS normalization.
    ...
    for _ in range(steps):
        ...
        B = b * A + c * A @ A
        X = a * X + B @ X
    # Per-row RMS normalization (NorMuon): balances per-neuron learning rates.
    row_rms = X.norm(dim=-1, keepdim=True) / math.sqrt(X.size(-1))
    X = X / row_rms.clamp_min(eps)
...
def eval_val_sliding(
    ...
    with torch.inference_mode():
        for batch_start in range(0, len(my_starts), batch_size):
            batch_starts = my_starts[batch_start:batch_start + batch_size]
            x_list, y_list = [], []
            ...
            # For each window, only score the last `stride` tokens
            # (except first window which scores all tokens).
            for i, s in enumerate(batch_starts):
                ...
                loss = F.cross_entropy(
                    scored_logits.float().reshape(-1, scored_logits.size(-1)),
                    scored_targets.reshape(-1),
                    reduction="sum",
                )
                num_scored = scored_targets.numel()
                val_loss_sum += loss.to(torch.float64)
                val_token_count += num_scored

                prev_ids = x[i, score_from:].reshape(-1)
...
# -----------------------------
# POST-TRAINING QUANTIZATION (INT6 + ZSTD)
# -----------------------------
...
INT6_MIN = -32
INT6_MAX = 31
INT6_SCALE_DENOM = 31.0
# Patterns for tensors that should be kept in fp16 (not quantized to int6).
# The tied embedding is critical — quantizing it causes disproportionate damage.
# Last 2 layers' key projections are also quantization-sensitive.
FP16_LARGE_PASSTHROUGH_PATTERNS = tuple(
    p for p in os.environ.get(
        "FP16_LARGE_PASSTHROUGH_PATTERNS",
        "tok_emb",
    ).split(",") if p
)
LATE_K_PASSTHROUGH_LAYERS = int(os.environ.get("LATE_K_PASSTHROUGH_LAYERS", 2))

def tensor_nbytes(t: Tensor) -> int:
    return int(t.numel()) * int(t.element_size())

def _is_fp16_passthrough(name: str, t: Tensor, num_layers: int) -> bool:
    """Check if a tensor should be kept in fp16 instead of quantized."""
    # Large tensors matching explicit passthrough patterns (e.g. tied embedding).
    if any(p in name for p in FP16_LARGE_PASSTHROUGH_PATTERNS):
        return True
    # Last N layers' key projections are quantization-sensitive.
    if LATE_K_PASSTHROUGH_LAYERS > 0 and ".c_k.weight" in name:
        for layer_idx in range(num_layers - LATE_K_PASSTHROUGH_LAYERS, num_layers):
...
def quantize_float_tensor_int6(t: Tensor) -> tuple[Tensor, Tensor]:
    """Quantize to int6 [-32, 31] stored in int8. Narrower range = better zstd compression."""
    t32 = t.float()
    if t32.ndim == 2:
        clip_abs = (
        ...
        scale = (clip_abs / INT6_SCALE_DENOM).clamp_min(1.0 / INT6_SCALE_DENOM)
    ...
    scale = torch.tensor(clip_abs / INT6_SCALE_DENOM if clip_abs > 0 else 1.0, dtype=torch.float32)
...
def fake_quantize_int6(w: Tensor) -> Tensor:
    """STE fake-int6 quantization for QAT. Forward uses quantized, backward flows through original."""
    abs_max = w.abs().amax(dim=-1, keepdim=True)
    scale = (abs_max / INT6_SCALE_DENOM).clamp_min(1.0 / INT6_SCALE_DENOM)
    w_q = (w / scale).round().clamp(INT6_MIN, INT6_MAX) * scale
    return w + (w_q - w).detach()

def quantize_state_dict_int6(state_dict: dict[str, Tensor], num_layers: int = 9):
    """Int6 quantization with FP16 passthrough for sensitive tensors."""
    ...
    for name, tensor in state_dict.items():
        ...
        # FP16 passthrough for sensitive tensors (tied embedding, late-layer keys).
        if _is_fp16_passthrough(name, t, num_layers):
            if t.dtype in {torch.float32, torch.bfloat16}:
                passthrough_orig_dtypes[name] = str(t.dtype).removeprefix("torch.")
...
class CastedLinear(nn.Linear):
    ...
    # With QAT enabled, applies STE fake-int6 quantization during training.
...
class SmearGate(nn.Module):
    """Learned sigmoid gate blending each token's embedding with the previous token's.
    Injects bigram-level info before the first attention layer at ~512 params cost."""
...
class BigramHash(nn.Module):
    """Hash-based bigram embedding: injects token-pair info via a learned hash table."""
    def __init__(self, num_buckets: int, hash_dim: int, model_dim: int):
        super().__init__()
        self.num_buckets = num_buckets
        self.embedding = nn.Embedding(num_buckets, hash_dim)
        self.proj = CastedLinear(hash_dim, model_dim, bias=False)

    def forward(self, input_ids: Tensor) -> Tensor:
        prev_ids = torch.cat([input_ids[:, :1], input_ids[:, :-1]], dim=1)
        bigram_idx = (input_ids * 31 + prev_ids) % self.num_buckets
        h = self.embedding(bigram_idx)
        return self.proj(h)
...
class GPT(nn.Module):
    ...
    def _init_weights(self, num_layers: int) -> None:
        ...
        # Orthogonal init for all weight matrices + muP-scaled output projections.
        for name, module in self.named_modules():
            if isinstance(module, nn.Linear):
                if getattr(module, "_zero_init", False):
                    ...
                    # Scale output projections by 1/sqrt(2*num_layers) (muP).
                    if ".proj." in name or name.endswith(".proj"):
                        module.weight.data *= 1.0 / math.sqrt(2 * num_layers)
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        x = self._run_backbone(input_ids).reshape(-1, self.tok_emb.weight.size(1))
    ...
    def forward_logits(self, input_ids: Tensor) -> Tensor:
        """Return per-token logits (for sliding window eval). Not compiled."""
...
def main() -> None:
    ...
    # Also include bigram_hash projection in Muon matrix params.
    bigram_matrix_params = [
        p for name, p in base_model.bigram_hash.named_parameters()
        if p.ndim == 2
    ]
    matrix_params = [
        p
    ...
    # SmearGate param (1D) and bigram hash embedding go in scalar optimizer.
    scalar_params.append(base_model.smear_gate.gate)
    scalar_params.append(base_model.bigram_hash.embedding.weight)
    ...
    while True:
        ...
        if scale < 1.0 and args.swa_interval > 0 and step % args.swa_interval == 0:
            swa_checkpoints.append({k: v.detach().cpu().clone() for k, v in base_model.state_dict().items()})
            if len(swa_checkpoints) > args.swa_count:
    ...
    if swa_checkpoints:
        ...
        for key in swa_checkpoints[0]:
            stacked = torch.stack([cp[key].float() for cp in swa_checkpoints])
            avg_state[key] = stacked.mean(dim=0).to(swa_checkpoints[0][key].dtype)
        base_model.load_state_dict(avg_state, strict=True)
        del swa_checkpoints

    # -----------------------------
    # SERIALIZATION + ROUNDTRIP VALIDATION (INT6 + ZSTD)
    # -----------------------------
    ...
    quant_obj, quant_stats = quantize_state_dict_int6(base_model.state_dict(), num_layers=args.num_layers)
    ...
    if master_process:
        ...
        size_ok = (quant_file_bytes + code_bytes) <= 16_000_000
        log0(f"Fits in 16MB: {size_ok} ({16_000_000 - quant_file_bytes - code_bytes} bytes headroom)")
    ...
    # Standard (non-overlapping) eval on quantized model.
    torch.cuda.synchronize()
    ...
    # Sliding window eval on quantized model (the real competition metric).
    eval_sl = args.eval_seq_len if args.eval_seq_len > 0 else args.train_seq_len
    val_tokens_sw = load_validation_tokens(args.val_files, eval_sl)
    torch.cuda.synchronize()
